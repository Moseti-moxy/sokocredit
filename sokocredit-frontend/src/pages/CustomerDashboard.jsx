...
export default function CustomerDashboard() {
  ...
  return <AppShell title={`Welcome, ${user?.name?.split(' ')[0] || 'Customer'}`} subtitle="Your SokoCredit account overview">
    ...
    <section className="mt-6 rounded-2xl border border-brand-100 bg-white p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-display font-semibold text-slate-900">Loan requests</h2><p className="mt-1 text-sm text-slate-500">Track every application from review through disbursement.</p></div><span className="rounded-lg bg-brand-50 px-3 py-2 text-sm font-semibold text-brand-700">Estimated eligible: KES {money(estimatedLimit)}</span></div>{renewalNotice && <p role="status" className="mt-4 rounded-xl bg-brand-50 px-3 py-2 text-sm text-brand-800">{renewalNotice}</p>}{applications.length ? <div className="mt-4 grid gap-3">{applications.map((loan) => <article key={loan.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-brand-100 p-4"><div><span className={`rounded-full px-2 py-1 text-xs font-semibold ${loan.loanType === 'chama' ? 'bg-violet-100 text-violet-700' : 'bg-slate-100 text-slate-700'}`}>{loan.loanType === 'chama' ? 'Chama group loan' : 'Individual loan'}</span><p className="mt-2 font-semibold text-slate-900">KES {money(loan.amount)} <span className="font-normal text-slate-500">· {loan.purpose || 'Loan request'}</span></p><p className="mt-1 text-xs text-slate-500">Requested {loan.appliedAt} · {loan.duration} months · {loan.frequency}</p>{loan.rejectionReason && <p className="mt-2 text-xs text-red-600">Reason: {loan.rejectionReason}</p>}{loan.renewalRequested && <p className="mt-2 text-xs text-brand-700">Renewal request: KES {money(loan.renewalAmount)} for {loan.renewalDuration} months · Under review</p>}</div><div className="flex items-center gap-2"><span className={`rounded-full px-3 py-1 text-xs font-semibold ${loan.status === 'Rejected' ? 'bg-red-50 text-red-700' : loan.status === 'Pending' ? 'bg-amber-50 text-amber-700' : loan.status === 'Repaying' ? 'bg-brand-50 text-brand-700' : 'bg-slate-100 text-slate-700'}`}>{loan.status}</span>{loan.status === 'Repaying' && !loan.renewalRequested && <button type="button" onClick={() => openRenewal(loan)} className="rounded-lg border border-brand-200 px-3 py-2 text-xs font-semibold text-brand-700 hover:bg-brand-50">Request renewal</button>}</div></article>)}</div> : <p className="mt-4 text-sm text-slate-500">You have not submitted any loan requests yet.</p>}</section>
