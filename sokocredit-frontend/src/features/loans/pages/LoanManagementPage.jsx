...
export default function LoanManagementPage() {
  ...
  const recordRepayment = (loan, payment) => {
    ...
    setNotice({ type: 'success', message: `KES ${applied.toLocaleString()} repayment recorded via ${payment.method}.` })
  }
  const sendReminder = (loan, reminder) => { update(loan.id, { reminders: [...(loan.reminders || []), { ...reminder, sentAt: new Date().toISOString() }] }); setNotice({ type: 'success', message: `Payment reminder sent to ${loan.customer} via ${reminder.channel}.` }) }
