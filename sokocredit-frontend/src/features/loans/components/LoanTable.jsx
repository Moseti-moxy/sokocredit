...
export default function LoanTable({ loans }) {
  return <div className="overflow-hidden rounded-[7px] border border-[#e9ebe5]">
    ...
    {loans.length ? loans.map((loan) => {
      ...
      return <div className={`grid min-h-[92px] grid-cols-2 items-center gap-4 border-t border-[#eee] px-4 py-4 text-sm md:grid-cols-[1.5fr_.8fr_1.15fr_1fr_.9fr] md:gap-3.5 ${overdue ? 'border-l-4 border-l-[#d42824] pl-3' : ''}`} key={loan.id}>
